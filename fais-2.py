    ...
    def get_ticket(self):
        for host in self.hosts:
            try:
                ...
                oauth = acc.get("oauth_login_only")
    ...
    def login_by_ticket(self, ticket):
        for host in self.hosts:
            try:
                ...
                # فقط هنا نطبع headers + response
                print(f"\n[LOGIN {host}] HEADERS ->", dict(r.headers))
                print(f"[LOGIN {host}] RESPONSE ->", r.text)
...
if __name__ == "__main__":
    ...
    if oauth is True:
        ...
        # false => login مباشر + بعدها safe/auth
