...
class TikTokFlow:
    ...
    def build_headers(self, params):
        ...
        return {
            'User-Agent': f'com.zhiliaoapp.musically/{params["manifest_version_code"]} (Linux; Android 10)',
    ...
    def get_ticket(self):
        def worker(host):
            ...
            try:
                ...
                r = self.session.post(url, params=params, headers=headers, proxies=self.proxies, timeout=8)

                j = r.json()
                accs = j.get("data", {}).get("accounts", [])
                if accs:
                    acc = accs[0]
                    ticket = acc.get("passport_ticket")
                    auth = acc.get("oauth_login_only", True)
                    if ticket:
                        print(f"[LOOKUP HIT {host}]")
                        return ticket, auth
            except:
                pass
            return None
        ...
        with ThreadPoolExecutor(max_workers=30) as exe:
            futures = [exe.submit(worker, h) for h in self.hosts]
    ...
    def safe_verify(self, ticket):
        def worker(host):
            ...
            try:
                ...
                r = self.session.get(url, params=params, headers=headers, proxies=self.proxies, timeout=8)

                j = r.json()
                if j.get("data", {}).get("error_code") == 2029:
                    print(f"[SAFE OK {host}]")
                    return True
            except:
                pass
            return False
    ...
    def login(self, ticket):
        def worker(host):
            ...
            try:
                ...
                if j.get("message") == "success" or j.get("data", {}).get("error_code") == 2135:
                    print(f"\n🔥 LOGIN HIT {host}")
...
if __name__ == "__main__":
    ...
    ticket, auth_required = flow.get_ticket()
    ...
    print("🎫 Ticket:", ticket)
    print("🔐 AUTH:", auth_required)

    if auth_required is False:
        flow.login(ticket)
    else:
        if flow.safe_verify(ticket):
            flow.auth(ticket)
